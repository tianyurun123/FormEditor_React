...
// 图标
import { FormOutlined, EyeOutlined, CodeOutlined } from '@ant-design/icons';
// 美化滚动条组件
...
// Vue3代码预览
import VueCodePreview from "@/components/VueCodePreview";
import {
...
export default function Create() {
	...
	// 是否展示 Vue3 代码预览
	const [vuePreviewOpen, setVuePreviewOpen] = useState(false);
	// 是否拖拽到表单区域上方
	...
	/* 导出 Vue3 + Element-Plus 代码 */
	const handleExportVueCode = () => {
		const values = checkForm();
		if (!values) return;
		setFormData({
			...values,
	 ...
	 *  */
	/* 拖拽开始 - 设置拖拽数据 */
	const handleDragStart = (e: React.DragEvent<HTMLLIElement>, source: string, tag: string, index: number) => {
		const dragData: dragDataType = {
			source: source as dragDataType['source'],
			tag,
			index
		};
	...
	const handleDrop = (e: React.DragEvent) => {
		...
		try {
			...
			if (!jsonData) return;
			const dragData: dragDataType = JSON.parse(jsonData);

			if (dragData.source === 'base') {
				const item = baseCompList[dragData.index];
				if (item) handleSelectComponent(item, 'base');
			} else if (dragData.source === 'template') {
				const item = templateCompList[dragData.index];
				if (item) handleSelectComponent(item, 'template');
			} else if (dragData.source === 'common') {
				const item = mineUserComponent[dragData.index];
				if (item) handleAddCommonComponent(item);
			}
		} catch (err) {
			console.warn('拖拽数据解析失败', err);
		}
	};
		...
		<div className="create-form-page">
			{!editResult ?
				<div className="create-form-wrapper">
										...
										<ul className="component-render">
											{baseCompList.map((item, idx) => (
												<li key={'basic-comp-' + idx}
													draggable="true"
													className='hover-color border-color draggable-source'
													onClick={() => handleSelectComponent(item, 'base')}
													onDragStart={(e) => handleDragStart(e, 'base', item.tag, idx)}
												>
										...
										<ul className="component-render">
											{templateCompList.map((item, idx) => {
												return <li key={'template-comp-' + idx}
													draggable="true"
													className='common-li hover-color border-color draggable-source'
													onClick={() => handleSelectComponent(item, 'template')}
													onDragStart={(e) => handleDragStart(e, 'template', item.tag, idx)}>
													<span>{item.label}</span>
												</li>
											})}
										</ul>
												...
												mineUserComponent.length ?
													mineUserComponent.map((item, idx) => {
														return <li key={'mine-component-' + idx} title={item.label}
															...
															onDragStart={(e) => handleDragStart(e, 'common', item.config.tag, idx)}>
					...
					<section
						className={`form-content ${isDragOver ? 'drag-over-active' : ''}`}
						...
						<div className="form-config-page">
							...
							<div className="center-board">
								<Space size={14} className="operate-setting">
									...
									<Button type="link" icon={<CodeOutlined />} onClick={handleExportVueCode} className="vue-export-btn">导出Vue3代码</Button>
			...
			{/* ----------- 查看Vue3代码 ------------ */}
			<Drawer width='60%' open={vuePreviewOpen} placement="right" onClose={() => setVuePreviewOpen(false)}
				rootClassName="preview-drawer-container" className="preview-drawer-box json-drawer-box">
				<VueCodePreview formData={formData} open={vuePreviewOpen} onClose={() => setVuePreviewOpen(false)} />
			</Drawer>
			{/* ----------- 导出tsx文件 ------------- */}
