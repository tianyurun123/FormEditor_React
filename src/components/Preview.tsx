...
function Preview(props: propTypes) {
	...
	const initForm = (arr: Array<baseProps>, reset?: boolean) => {
		if (reset) {
			...
			// 初始化数据
			arr.forEach(item => {
				const data: { type: string, value: any, details: any } = {
					type: item.tag,
					value: undefined,
					details: undefined
				};
				if (item.tag === 'table') {
					...
					data.value = Array.from({ length: rowCount }, () => Array(colCount).fill(''));
				}
	...
	const renderShowDetailInput = (attr: string, optionList: Array<optionTypes>) => {
		...
		// 如果选中非其他内容项，就清除之前其他内容（有可能之前选中其他，现在修改选项）
	...
	/* 层级联动的文本占位符提示 */
	/* 表格单元格内容变化 */
	const tableCellChange = (attr: string, rowIdx: number, colIdx: number, value: string) => {
		if (!submitValues[attr]) {
			submitValues[attr] = { value: [], details: undefined };
		}
		if (!submitValues[attr].value) {
			submitValues[attr].value = [];
		}
		if (!submitValues[attr].value[rowIdx]) {
			submitValues[attr].value[rowIdx] = [];
		}
		submitValues[attr].value[rowIdx][colIdx] = value;
		// 清除验证错误状态
		if (attr in ruleList && ruleList[attr].error) {
			ruleList[attr].error = false;
		}
				...
				{renderFormList.map((item, idx) => (
					<div key={item.id} id={'write-question-' + item.id} className='write-item'>
						...
						<>
							...
							{item.type === 'cascader' ?
								<div className='space-wrapper'>
									<Cascader column={item.levelCount} options={item.tag === 'address' ? options : item.options}
...
n									{/* --------------- 表格/矩阵 ----------------- */}
									{item.tag === 'table' ?
										<div className="write-table-wrapper">
											<table className="write-table" cellSpacing="0" cellPadding="0">
												<thead>
													<tr>
														{item.showRowLabel ? <th className="write-table-corner"></th> : null}
														{(item.columns || []).map((col: any, cIdx: number) => (
															<th key={col.id}>{col.label || `列${cIdx + 1}`}</th>
														))}
													</tr>
												</thead>
												<tbody>
													{Array.from({ length: item.rowCount || 3 }, (_, rIdx) => (
														<tr key={rIdx}>
															{item.showRowLabel ? (
																<td className="write-table-row-label">
																	{item.rowLabels?.[rIdx] || `行${rIdx + 1}`}
																</td>
															) : null}
															{(item.columns || []).map((col: any, cIdx: number) => {
																const cellVal = (submitValues[item.id]?.value?.[rIdx]?.[cIdx]) || '';
																return (
																	<td key={col.id}>
																		<Input
																			value={cellVal}
																			onChange={e => tableCellChange(item.id, rIdx, cIdx, e.target.value)}
